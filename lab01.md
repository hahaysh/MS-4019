...
- Copilot Chat 기본 사용 경험
- 파일 기반 분석 경험
- 웹 + 문서 기반 리서치 경험
- Prebuilt Agent 활용 경험
...
Copilot Chat이 일반 검색과 어떻게 다른지 확인

### UI 빠른 둘러보기

- 새 채팅: 이전 맥락과 분리된 새 대화 시작
- 검색: 이전 대화 및 메시지 빠르게 찾기
- 라이브러리: 사용 가능한 Agent와 관련 항목 확인

### 강사 진행

- 새 채팅을 눌러 데모용 대화를 시작
- 검색에서 이전 대화 키워드 검색 기능 확인
- 라이브러리에서 Prompt Coach 위치 확인

### 비교 실습 예제 (검색 vs 라이브러리 vs 새 채팅)
...
#### 2) 후속 Prompt로 대화 컨텍스트 확인

```text
방금 답변을 IT 헬프데스크 신입용 체크리스트로 바꿔줘.
```

#### 3) 검색으로 과거 대화 찾기

- 검색에서 `보안` 또는 `헬프데스크` 키워드 입력
- 방금 만든 대화가 검색 결과에 나타나는지 확인
- 검색은 답변 생성이 아니라 과거 대화 탐색용임을 확인

#### 4) 라이브러리에서 Agent 전환

- 라이브러리에서 Prompt Coach 선택
- 동일 주제로 Prompt 개선 요청

```text
"Microsoft 365 Copilot의 보안 관련 기능을 3가지로 요약해줘" 프롬프트를 더 구체적으로 개선해줘.
```
...
- 후속 질문 활용
- 검색이 아닌 대화형 AI
- 검색은 과거 대화 탐색 기능이며, 답변 생성은 새 채팅/대화에서 진행
- 라이브러리는 Agent 진입점으로 활용 가능
...
- Copilot Chat은 단순 검색 도구가 아니라 대화형 AI 도우미이다.
- 파일을 업로드하면 문서 기반 분석과 요약이 가능하다.
- 웹 정보와 문서 정보를 함께 활용할 수 있다.
- Prebuilt Agent는 특정 업무 목적에 최적화된 Copilot 경험을 제공한다.
- 좋은 결과를 얻기 위해서는 질문하는 방법도 중요하다.